...
import { buildJournalArticleAbstractPath, getJournalRouteSlug, slugifyTitle } from "../../utils/journalLinks";
  ...
  for (const year of journal?.archive || []) {
    for (const volume of year.volumes || []) {
      for (const issue of volume.issues || []) {
        const match = (issue.articles || []).find((item) => matchesArticle(item, articleId));
...
export default function ArticlePdfViewerPage() {
  ...
  const abstractPath = buildJournalArticleAbstractPath(journalUrl, articleId, articleTitle);
